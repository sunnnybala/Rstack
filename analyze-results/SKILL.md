...
```bash
mkdir -p .rstack/analysis/figures .rstack/analysis/tables .rstack/analysis/scripts
```
...
2. Read `.rstack/results/` directory to find raw outputs (metrics.json, figures/, stdout.log for each run).
...
Write table source to `.rstack/analysis/tables/comparison.tex`.
...
import json
# Read metrics, plot loss/accuracy curves, save to .rstack/analysis/figures/
```
...
For each figure:
1. Write a Python script to `.rstack/analysis/scripts/fig_{name}.py`
2. Run it: `python .rstack/analysis/scripts/fig_{name}.py`
3. Output PNG + PDF to `.rstack/analysis/figures/`

If matplotlib is not installed, run `pip install matplotlib` first.

## Step 3: Statistical Summary

Write `.rstack/analysis/stats.json` with:
```json
{
  "total_runs": 5,
  "completed_runs": 4,
